...
movieModel.find({}, function(err, movies){
   if (err){
    ...
    for (let movie of movies) {
     moviesList.push({id: movie._id, name: movie.name, released_on: movie.released_on, movie_image: movie.movieImage});
    }
...
create: function(req, res, next) {
  movieModel.create({ name: req.body.name, released_on: req.body.released_on, movieImage : req.files.map(file => file.path) },
   function (err, result) {
...
// movieImage : req.files.map(file => file.path)
